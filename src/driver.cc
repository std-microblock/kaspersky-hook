...
EXTERN_C NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING) {
    ...
    const auto success = kaspersky::hook_ssdt_routine(
        NtCreateFile_index, &hooks::hk_NtCreateFile,
...
void DriverUnload(PDRIVER_OBJECT DriverObject) {
    ...
    //
    // Delay execution to make sure no thread is executing our hook. (Better
